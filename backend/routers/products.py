            ...
            if page_ids:
                ...
                rows = conn.execute(
                    f"""SELECT p.id, p.name, p.name_display, p.lifecycle, p.popularity_score,
                               ...
                               p.unit, p.price_usd, p.price_uzs, p.weight, p.package_quantity, p.whole_package_only, p.image_path,
...
def get_products_by_ids(ids: str = Query(..., description="Comma-separated product IDs")):
    ...
    rows = conn.execute(
        f"""SELECT p.id, p.name, p.name_display, p.unit,
                   p.price_usd, p.price_uzs, p.package_quantity, p.whole_package_only, p.image_path,
                   p.stock_quantity, p.stock_status
            FROM products p
            WHERE p.id IN ({placeholders})""",
        id_list,
