...
def test_register_creates_new_whitelist_row(db):
    _make_agent(db, 1001)
    r = register_new_shop(
        db, 1001, "Sardor", "Khasanov", "Taxta bozor, Sochak",
        "+998 90 111 22 33", 41.31, 69.27,
    )
    assert r["status"] == "created"
    assert r["client_id"] > 0

    row = db.execute(
        "SELECT name, moljal, phone_normalized, source_sheet, segment, "
        "gps_latitude, gps_longitude, gps_set_by_role, gps_set_by_tg_id "
        "FROM allowed_clients WHERE id = ?",
        (r["client_id"],),
    ).fetchone()
    assert row["name"] == "Sardor Khasanov"
    assert row["moljal"] == "Taxta bozor, Sochak"
...
def test_register_writes_audit_row(db):
    _make_agent(db, 1002)
    r = register_new_shop(
        db, 1002, "Aygul", "Yusupova", "Chorsu bozor, 5-darvoza",
        "+998901234567", 41.0, 69.0,
    )
    assert r["status"] == "created"
    audit = db.execute(
        "SELECT agent_telegram_id, shop_name, first_name, last_name, venue, "
        "phone_normalized, status, linked_client_id "
        "FROM agent_client_registrations ORDER BY id DESC LIMIT 1"
    ).fetchone()
    assert audit["agent_telegram_id"] == 1002
    assert audit["shop_name"] == "Aygul Yusupova"
    assert audit["first_name"] == "Aygul"
    assert audit["last_name"] == "Yusupova"
    assert audit["venue"] == "Chorsu bozor, 5-darvoza"
...
def test_register_phone_collision_links_existing_row(db):
    ...
    r = register_new_shop(
        db, 1003, "New", "Person", "Some venue",
        "+998 90 555 44 33", 41.0, 69.0,
    )
...
def test_register_collision_matches_secondary_phone_slot(db):
    ...
    r = register_new_shop(
        db, 1004, "Other", "Person", "Some place",
        "+998 90 777 77 77", 41.0, 69.0,
    )
    assert r["status"] == "linked_existing"
    assert r["client_id"] == existing_id
...
def test_register_validates_inputs(db):
    _make_agent(db, 1005)
    # Short first name
    r = register_new_shop(db, 1005, "S", "Khasanov", "Taxta bozor", "+998901234567", 41.0, 69.0)
    assert r["status"] == "failed"
    # Short last name
    r = register_new_shop(db, 1005, "Sardor", "K", "Taxta bozor", "+998901234567", 41.0, 69.0)
    assert r["status"] == "failed"
    # Short venue
    r = register_new_shop(db, 1005, "Sardor", "Khasanov", "T", "+998901234567", 41.0, 69.0)
    assert r["status"] == "failed"
    # Phone too short
    r = register_new_shop(db, 1005, "Sardor", "Khasanov", "Taxta bozor", "12345", 41.0, 69.0)
    assert r["status"] == "failed"
    # Missing GPS
    r = register_new_shop(db, 1005, "Sardor", "Khasanov", "Taxta bozor", "+998901234567", None, None)
    assert r["status"] == "failed"
...
def test_register_skips_merged_clients_in_collision_check(db):
    ...
    r = register_new_shop(
        db, 1006, "Active", "Customer", "Yangi joy",
        "+998 90 888 88 88", 41.0, 69.0,
    )
    assert r["status"] == "created"
